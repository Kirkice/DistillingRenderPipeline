...
public class CopyTangentFeature : ScriptableRendererFeature
{
    class CopyTangentPass : ScriptableRenderPass
    {
        private Material _material;
        public int soildColorID = 0;
        public ShaderTagId shaderTag = new ShaderTagId("UniversalForward");
        const string m_ProfilerTag = "CopyTangent Pass"; 
        ProfilingSampler m_ProfilingSampler = new ProfilingSampler(m_ProfilerTag);
        FilteringSettings filtering;
        public CopyTangentPass()
        {
            filtering = new FilteringSettings(RenderQueueRange.opaque, 1 << LayerMask.NameToLayer("Default"));
            _material = new Material(Shader.Find("Hidden/Distilling RP/CopyTangentWS"));
        }

        public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
        {
            int temp = Shader.PropertyToID("_CameraTangentTexture");
    ...
    CopyTangentPass m_ScriptablePass;

    public override void Create()
    {
        m_ScriptablePass = new CopyTangentPass();
