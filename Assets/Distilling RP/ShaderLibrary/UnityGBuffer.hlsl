...
FragmentOutput SurfaceDataToGbuffer(SurfaceDataGBuffer surfaceData, InputData inputData, half3 globalIllumination, int lightingMode)
{
...
SurfaceDataGBuffer SurfaceDataFromGbuffer(half4 gbuffer0, half4 gbuffer1, half4 gbuffer2, int lightingMode)
{
    SurfaceDataGBuffer surfaceData;
...
InputData InputDataFromGbufferAndWorldPosition(half4 gbuffer2, float3 wsPos)
{
    ...
    inputData.normalWS = normalize(gbuffer2.xyz * 2 - 1); // normalize() is required because terrain shaders use additive blending for normals (not unit-length anymore)
