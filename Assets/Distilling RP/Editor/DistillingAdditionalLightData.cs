...
namespace UnityEditor.Rendering.Distilling
{
    ...
    [CustomEditorForRenderPipeline(typeof(Light), typeof(DistillingRenderPipelineAsset))]
...
#pragma warning restore 618
        ...
        DistillingAdditionalLightData m_AdditionalLightData;
        ...
        {
            m_AdditionalLightData = lightProperty.gameObject.GetComponent<DistillingAdditionalLightData>();
            settings.OnEnable();
            init(m_AdditionalLightData);
            UpdateShowOptions(true);
        }

        void init(DistillingAdditionalLightData additionalLightData)
        {
        ...
        void DrawAdditionalShadowData()
        {
            ...
            if (hasChanged)
            {
                if (m_AdditionalLightDataSO == null)
                {
                    lightProperty.gameObject.AddComponent<DistillingAdditionalLightData>();
                    m_AdditionalLightData = lightProperty.gameObject.GetComponent<DistillingAdditionalLightData>();
        ...
        {
            if (!(GraphicsSettings.currentRenderPipeline is DistillingRenderPipelineAsset))
                return;
