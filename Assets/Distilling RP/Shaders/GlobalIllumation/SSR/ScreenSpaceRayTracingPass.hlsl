...
float4 PS_Resolve (VertexOutput pin) : SV_Target
{
   ...
   for(int i = 0; i < NumResolve; i++)
   {
            ...
            float4 hitPacked													= SAMPLE_TEXTURE2D_LOD(_RayCast, sampler_RayCast,neighborUv,1);
            float2 hitUv														= hitPacked.xy;
            float hitZ															= hitPacked.z;
            float hitPDF														= hitPacked.w;
			float hitMask														= SAMPLE_TEXTURE2D_LOD(_RayCastMask, sampler_RayCastMask,neighborUv,1).r;
...
{	
	...
	float2 jitter																= SAMPLE_TEXTURE2D_LOD(_Noise, sampler_Noise,float4((uv + _JitterSizeAndOffset.zw) * _RayCastSize.xy / _NoiseSize.xy, 0, -255),1); // Blue noise generated by https://github.com/bartwronski/BlueNoiseGenerator/;
...
/// PS_RayCastMask
/// </summary>
float4 PS_RayCastMask (VertexOutput pin) : SV_Target
{	
	float2 uv																	= pin.TexC;
	int2 pos																	= uv;
...
{	 
	...
	return reflection * mask * 0.1 + sceneColor;
}
